...
public class WrapperMethods {
	...
	// Only for Android
	public boolean stopRunningAppInAndroid(String appPackage, String appActivity) {
		try {
	...
	public boolean launchChromeBrowser(String browserName, String deviceName, String URL) {
		try {
			DesiredCapabilities dc = new DesiredCapabilities();
	...
	// Not available for iOS
	public void turnBluetoothOffInAndroid() {
		Map<String, Object> args = new HashMap<>();
		args.put("command", "service");
		args.put("args", "call bluetooth_manager 9");
		Object executeScript = driver.executeScript("mobile:shell", args);
		System.out.println(executeScript.toString());
	}

	// Not available for iOS
	public void turnBluetoothOnInAndroid() {
		Map<String, Object> args = new HashMap<>();
		args.put("command", "service");
		args.put("args", "call bluetooth_manager 6");
		Object executeScript = driver.executeScript("mobile:shell", args);
		System.out.println(executeScript.toString());
	}
	...
	public void turnLocationOffInAndroid() {
		...
		args.put("args", "put secure location_providers_allowed -gps");
		Object executeScript = driver.executeScript("mobile:shell", args);
		System.out.println(executeScript.toString());
	}
	...
	public boolean pressBack() {
		// ((PressesKey) driver).pressKeyCode(4);
	...
	public void doubleTapUsingPointerInput() {
		Dimension size = driver.manage().window().getSize();
		int x1 = (int) (size.getWidth() * 0.5);
		int y1 = (int) (size.getHeight() * 0.5);
		PointerInput finger = new PointerInput(PointerInput.Kind.TOUCH, "finger1");
		Sequence tap = new Sequence(finger, 1);
		tap.addAction(finger.createPointerMove(Duration.ofMillis(0), PointerInput.Origin.viewport(), x1, y1));
	...
	public void doubleTapUsingTouchActions() {
		Dimension size = driver.manage().window().getSize();
		int x1 = (int) (size.getWidth() * 0.5);
		int y1 = (int) (size.getHeight() * 0.5);
		MultiTouchAction touch = new MultiTouchAction(driver);
		touch.add(new TouchAction<>(driver).press(point(x1, y1)).release()
				.waitAction(WaitOptions.waitOptions(Duration.ofMillis(200))).press(point(x1, y1)).release()).perform();
	...
	public boolean scrollFromUpToDowninAppWithWebElement(WebElement ele) {
		try {
			// ((MobileElement) ele).getCenter().getX();
			// ((MobileElement) ele).getCenter().getY();
	...
	// Only For iOS
	public boolean launchAppInIOs(String deviceName, String DeviceID, String xcodeSignedId, String appPath,
			String bundleId) {
		try {
			...
			caps.setCapability("xcodeConfigFile", xcodeSignedId);
	...
	// For iOS only
	public boolean launchSafariBrowser(String browserName, String deviceName,String platformVersion, String URL) {
		try {
			DesiredCapabilities dc = new DesiredCapabilities();
			dc.setCapability("browserName", browserName);
			dc.setCapability("deviceName", deviceName);
	...
	// For iOS only
	public boolean launchAppInIOS(String deviceName, String DeviceID, String xcodeOrgId, String appPath,
			String bundleId, String platformVersion) {
		try {
			DesiredCapabilities caps = new DesiredCapabilities();
			caps.setCapability("platformName", "iOS");
			caps.setCapability("deviceName", deviceName);
	...
	// Only For iOS
	public boolean launchAppInIOsSimulator(String deviceName,String platformVersion, String appPath) {
		try {
			DesiredCapabilities caps = new DesiredCapabilities();
			caps.setCapability("deviceName", deviceName);
			caps.setCapability("platformVersion", platformVersion);
	...
	public boolean launchAppInIOsSimulatorwithBundleId(String deviceName,String platformVersion,String bundleId) {
		try {
			DesiredCapabilities caps = new DesiredCapabilities();
			caps.setCapability("deviceName", deviceName);
			caps.setCapability("platformVersion", platformVersion);
	...
	public boolean WifiOffInIOS() {
		...
		try {
			...
			if (value.equals("0"))
				click(driver.findElementByXPath("//XCUIElementTypeSwitch"));
	...
	public void switchBetweenAppInIos(String bundleID) {
		driver.activateApp(bundleID);
	}
	
	public void stopRunningIosApp(String bundleID) {
